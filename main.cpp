...
void UpdateBitmapPixels() {
    ...
    if(got_para==false){
		...
		else
		{
			...
			{
				lsbmp.clear();
				lsbmp.draw_line(0,0,ylzx,ylzy,RGBPixel(0,255,0));
			}
			else if(ylzp==1)
			{
				GetCursorPos(&mouse_start);
				...
				memcpy(lsbmp.p,old_show.p,lsbmp.wid*lsbmp.hei*3);
				tmpy=tmpy*lsbmp.wid;
				tmpys=tmpys*lsbmp.wid;
				tmpyl=tmpyl*lsbmp.wid;
				for(int j=tmpys;j<tmpyl;j++)
				{
					lsbmp.p[j].R=255-lsbmp.p[j].R;
					lsbmp.p[j].G=255-lsbmp.p[j].G;
					lsbmp.p[j].B=255-lsbmp.p[j].B;
				}
			}
		}
    }
...
void render_bg(){
	...
	bg_rgbd.read("bg_rgbd.bmp");
	a.cutfrom(bg_rgbd,0,0,bg_rgbd.wid/2,bg_rgbd.hei);
	dep.cutfrom(bg_rgbd,bg_rgbd.wid/2,0,bg_rgbd.wid,bg_rgbd.hei);
	bg.wid=b.wid;
	bg.hei=b.hei;
	bg.space_apply();
	delete[] bg_rgbd.p;
	...
	double stadep=127.0;
	double maxdep=1600;
	a.resize((a.wid*2560+a.hei/2)/a.hei,2560);
	dep.resize(a.wid,a.hei);
	int xpy=(a.wid-1440)/2;
	
    omp_set_num_threads(12);
    #pragma omp parallel for shared(a, dep, xpy, stadep, maxdep) schedule(dynamic)
	for(int i=0;i<bg.hei;i++)
	{
		for(int j=0;j<bg.wid;j++)
		{
			double pian;
			double x_st,x_ed,now_depth,dp;
			double s1,s2,dd,t_x;
			int dx,stx,edx,x;
			
			//pian=(20-b.p[i*b.wid+j].R)*0.7279404685*maxdep/255/40;//tan(20^)*2
			pian=(20-b.p[i*b.wid+j].R)*0.1141867402;
			if(pian==0)
			{
				bg.p[i*bg.wid+j].R=a.p[i*a.wid + xpy+j].R;
			}
			else
			{
				x_st =xpy + j - pian*(255-stadep);
				x_ed =xpy + j + pian*stadep;
				if(pian>=0)dx=1;
				else dx=-1;
				stx = x_st - dx;
				edx = x_ed + 2*dx;
				if(stx<0)stx=0;
				if(stx>a.wid-1)stx=a.wid-1;
				if(edx<0)edx=0;
				if(edx>=a.wid)edx=a.wid-1;
				x=stx;
				do
				{
					x+=dx;
					now_depth = (x_ed-x)/(x_ed-x_st)*255;
					if(dep.p[i*dep.wid+x].R>=now_depth)
					{
						x-=dx;
						//x1=x_st y1=255 x2=x_ed y2=0
						//x3=x y3=dep.p[i*dep.wid+x].R x4=x+dx y4=dep.p[i*dep.wid+x+dx].R
						s1=-255*x_ed;
						s2=x*dep.p[i*dep.wid+x+dx].R-(x+dx)*dep.p[i*dep.wid+x].R;
						dd=(x_st-x_ed)*(dep.p[i*dep.wid+x].R-dep.p[i*dep.wid+x+dx].R)+255*dx;
						t_x=(s1*(-dx)-s2*(x_st-x_ed))/dd;
						bg.p[i*bg.wid+j].R=a.getp_r(t_x,i);
						break;
					}
				}while(x!=edx);
			}
			
			pian=(20-b.p[i*b.wid+j].G)*0.1141867402;
			if(pian==0)
			{
				bg.p[i*bg.wid+j].G=a.p[i*a.wid + xpy+j].G;
			}
			else
			{
				x_st =xpy + j - pian*(255-stadep);
				...
				do
				{
					...
					if(dep.p[i*dep.wid+x].R>=now_depth)
					{
						...
						dd=(x_st-x_ed)*(dep.p[i*dep.wid+x].R-dep.p[i*dep.wid+x+dx].R)+255*dx;
						t_x=(s1*(-dx)-s2*(x_st-x_ed))/dd;
						bg.p[i*bg.wid+j].G=a.getp_g(t_x,i);
						break;
					}
				}while(x!=edx);
			}
			
			pian=(20-b.p[i*b.wid+j].B)*0.1141867402;
			if(pian==0)
			{
				bg.p[i*bg.wid+j].B=a.p[i*a.wid + xpy+j].B;
			}
			else
			{
				x_st =xpy + j - pian*(255-stadep);
				...
				do
				{
					...
					if(dep.p[i*dep.wid+x].R>=now_depth)
					{
						...
						dd=(x_st-x_ed)*(dep.p[i*dep.wid+x].R-dep.p[i*dep.wid+x+dx].R)+255*dx;
						t_x=(s1*(-dx)-s2*(x_st-x_ed))/dd;
						bg.p[i*bg.wid+j].B=a.getp_b(t_x,i);
						break;
					}
				}while(x!=edx);
			}
...
void init(){
	...
	if (env_path != NULL) {
		snprintf(appdata_path, sizeof(appdata_path), "%s", env_path);
		snprintf(file_path, sizeof(file_path), "%s\\D20_Dice\\data", appdata_path);
		snprintf(bg_path, sizeof(bg_path), "%s\\D20_Dice\\bg.bmp", appdata_path);
	}
	else{
...
void measure_length(){
    ...
    omp_set_num_threads(12);
    // 简单的并行化：只对最外层i循环并行
    #pragma omp parallel for private(hsvtmp, jg, pp) schedule(dynamic)
    for(int i=0;i<2560;i++)
    {
    	...
    	for(int j=0;j<1440;j++)
    	{
    		...
    		old_show.p[i*old_show.wid+j]=hsv2rgb(hsvtmp);
    	}
    }
    ...
    switch(msg) {
        ...
        case WM_KEYDOWN: {
            if (wParam == VK_ESCAPE) {
				...
				else if(got_para==true&&ticks==0&&bDragging==false)
				{
					ticks=1;
					movex[0]=0;
					movey[0]=0;
					dice_ori.rotate(vec3d(0,-1,0),Pi/120);
					update=true;
				}
			}
				...
				else if(got_para==true&&ticks==0&&bDragging==false)
				{
					...
					movex[0]=0;
					movey[0]=0;
					dice_ori.rotate(vec3d(-1,0,0),Pi/120);
					update=true;
				}
			}
