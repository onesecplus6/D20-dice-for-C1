...
#include <time.h>  // ÓÃÓÚ srand ºÍ time
#include <stdlib.h> // ÓÃÓÚ rand
#include <math.h>
#include "bmp.h"
#include "stl.h"
#include "charedit.h"
...
int g_bitsPerPixel = 24; // 32Î» BGRA (»òRGBA£¬È¡¾öÓÚÏµÍ³ºÍDIB_RGB_COLORSµÄÄ¬ÈÏ½âÊÍ)
int windowX, windowY;
...
// ¶¯Ì¬±ä»¯µÄ²ÎÊý£¬ÀýÈç£¬ÓÃÓÚÉú³ÉÑÕÉ«
int g_colorOffset = 0;
const int TIMER_ID = 1; // ¼ÆÊ±Æ÷ID
RGBPixel rgbp;
HSVPixel hsvp;
// º¯ÊýÉùÃ÷
...
// ¶¨ÒåÒ»¸ö½á¹¹ÌåÀ´´æ´¢ÕÒµ½µÄÏÔÊ¾Æ÷ÐÅÏ¢
typedef struct MonitorInfo {
	HMONITOR hMonitor;
	RECT     rcMonitor;	// ¼àÊÓÆ÷µÄÕû¸öÏÔÊ¾ÇøÓò
    RECT     rcWork;	   // ¼àÊÓÆ÷µÄ¹¤×÷ÇøÓò£¨²»°üÀ¨ÈÎÎñÀ¸µÈ£©
    BOOL     bPrimary;     // ÊÇ·ñÊÇÖ÷ÏÔÊ¾Æ÷
} MonitorInfo;

// ´æ´¢ËùÓÐÏÔÊ¾Æ÷µÄÐÅÏ¢
MonitorInfo g_monitors[16]; // ¼ÙÉè×î¶à16¸öÏÔÊ¾Æ÷
int g_monitorCount = 0;

// EnumDisplayMonitors µÄ»Øµ÷º¯Êý
BOOL CALLBACK MonitorEnumProc(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData) {
    ...
    return TRUE; // ¼ÌÐøÃ¶¾ÙÏÂÒ»¸ö¼àÊÓÆ÷
}
// ´´½¨ DIB Section
// ÐÞ¸Ä CreateDIBSectionBitmap º¯ÊýÒÔ¼ÆËã stride
HBITMAP CreateDIBSectionBitmap(HDC hdc, int width, int height, int bitsPerPixel, void** pBits) {
    BITMAPINFO bmi;
    ZeroMemory(&bmi, sizeof(BITMAPINFO));
    bmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
    bmi.bmiHeader.biWidth = width;
    bmi.bmiHeader.biHeight = -height; // ¸º¸ß¶È±íÊ¾¶¥µ½µ×µÄDIB
    bmi.bmiHeader.biPlanes = 1;
    bmi.bmiHeader.biBitCount = bitsPerPixel;
    bmi.bmiHeader.biCompression = BI_RGB; // ÎÞÑ¹Ëõ

    // ¼ÆËã stride (Ã¿ÐÐµÄÊµ¼Ê×Ö½ÚÊý£¬°üÀ¨Ìî³ä)
    // ¶ÔÓÚ BI_RGB Ñ¹Ëõ£¬Windows GDI »á×Ô¶¯½øÐÐ 4 ×Ö½Ú¶ÔÆë
    int bytesPerPixel = bitsPerPixel / 8;
    g_bitmapStride = ((width * bytesPerPixel + 3) / 4) * 4; // È·±£ÊÇ4µÄ±¶Êý
    ...
    for(int i=0;i<40;i++)
    {
        ...
        for(int j=0;j<dice_pre[i].number;j++)
        {
			...
			for (int yy = y_from; yy <= y_to; ++yy) {
				double x_left = INFINITY;   // ×ó±ß½ç³õÊ¼»¯ÎªÕýÎÞÇî
				double x_right = -INFINITY; // ÓÒ±ß½ç³õÊ¼»¯Îª¸ºÎÞÇî
				// ´¦ÀíÈýÌõ±ß
				for (int ii = 0; ii < 3; ++ii) {
					...
					// ¼ì²éµ±Ç° y ÊÇ·ñÔÚ±ßµÄ×Ý×ø±ê·¶Î§ÄÚ£¨º¬¶Ëµã£©
					if (yy < ya - EPS || yy > yb + EPS)
						continue;
					// ´¦ÀíË®Æ½±ß
					if (fabs(ya - yb) < EPS) {
						if (fabs(yy - ya) < EPS) { // y Ç¡ºÃµÈÓÚ±ßµÄ×Ý×ø±ê
					...
					// ÆÕÍ¨±ß£¬ÏßÐÔ²åÖµÇó½»µã
						...
						double tt = (yy - ya) / (yb - ya); // t ÔÚ [0,1] ÄÚ
				...
				// ¸ù¾Ý×óÓÒ±ß½çÊä³öÕûÊýµã
				if (x_left <= x_right + EPS) { // ÓÐÐ§Çø¼ä
...
void UpdateBitmapPixels() {
    ...
    int bytesPerPixel = g_bitsPerPixel / 8; // ÀýÈç£¬¶ÔÓÚ 24 Î»£¬bytesPerPixel = 3
    if(got_para==false){
...
HBITMAP copybmp(HDC hdc, bmp24 input){
    ...
    for (int y = 0; y < g_bitmapHeight; ++y) {
        int lineStart = y * g_bitmapStride;
        for (int x = 0; x < g_bitmapWidth; ++x) {
            int index = lineStart + x * 3; // ÔÚµ±Ç°ÐÐÄÚµÄÆ«ÒÆÁ¿
            int loc=(g_bitmapHeight-1-y)*g_bitmapWidth+x;
...
public:
    ...
    bool connect() {
        hPipe = CreateFileA(
            pipeName.c_str(),           // ¹ÜµÀÃû³Æ
            GENERIC_READ | GENERIC_WRITE, // ¶ÁÐ´È¨ÏÞ
            FILE_SHARE_READ | FILE_SHARE_WRITE, // ¹²ÏíÄ£Ê½
            NULL,                       // Ä¬ÈÏ°²È«ÊôÐÔ
            OPEN_EXISTING,              // ´ò¿ªÒÑ´æÔÚµÄ¹ÜµÀ
        ...
        if (hPipe == INVALID_HANDLE_VALUE) 
		{
            ...
            /*
            std::cerr << "ÎÞ·¨Á¬½Óµ½¹ÜµÀ: " << pipeName << std::endl;
            std::cerr << "´íÎó´úÂë: " << error << std::endl;
            if (error == ERROR_PIPE_BUSY){std::cerr << "¹ÜµÀÕýÃ¦£¬ÇëÉÔºóÖØÊÔ" << std::endl;}
			else if (error == ERROR_FILE_NOT_FOUND) {std::cerr << "¹ÜµÀ²»´æÔÚ£¬ÇëÈ·±£·þÎñÆ÷ÕýÔÚÔËÐÐ" << std::endl;}
			else if (error == ERROR_ACCESS_DENIED) {std::cerr << "·ÃÎÊ±»¾Ü¾ø" << std::endl;}
			*/
            return false;
        }
    ...
    bool sendCommand(const std::string& command, std::string& response) {
        if (hPipe == INVALID_HANDLE_VALUE) {
            std::cerr << "Î´Á¬½Óµ½¹ÜµÀ" << std::endl;
            return false;
        }
        DWORD bytesWritten;
        BOOL writeResult = WriteFile(
            hPipe,                      // ¹ÜµÀ¾ä±ú
            command.c_str(),            // Òª·¢ËÍµÄÊý¾Ý
            static_cast<DWORD>(command.length()), // Êý¾Ý³¤¶È
            &bytesWritten,              // Êµ¼ÊÐ´ÈëµÄ×Ö½ÚÊý
            NULL                        // ·ÇÖØµþI/O
        );
        if (!writeResult) {
            std::cerr << "·¢ËÍÃüÁîÊ§°Ü£¬´íÎó´úÂë: " << GetLastError() << std::endl;
            return false;
        }
        char buffer[2048];
        DWORD bytesRead;
        BOOL readResult = ReadFile(
            hPipe,                      // ¹ÜµÀ¾ä±ú
            buffer,                     // ½ÓÊÕ»º³åÇø
            sizeof(buffer) - 1,         // »º³åÇø´óÐ¡
            &bytesRead,                 // Êµ¼Ê¶ÁÈ¡µÄ×Ö½ÚÊý
            NULL                        // ·ÇÖØµþI/O
        );
        if (readResult && bytesRead > 0) {
            ...
            DWORD error = GetLastError();
            std::cerr << "¶ÁÈ¡ÏìÓ¦Ê§°Ü£¬´íÎó´úÂë: " << error << std::endl;
            return false;
        }
    }
};
...
int pipes(){
    ...
    if (!client.connect()) {
        //std::cerr << "Á¬½ÓÊ§°Ü£¬ÇëÈ·±£·þÎñÆ÷¶ËÕýÔÚÔËÐÐ" << std::endl;
        return 1;
    }
    std::string response;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    HWND hwnd;
    MSG Msg;
    srand((unsigned int)time(NULL)); // ³õÊ¼»¯Ëæ»úÊýÉú³ÉÆ÷
    ...
    wc.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1); // ¿ÉÒÔ²»Ê¹ÓÃ£¬ÒòÎª»á±»Î»Í¼¸²¸Ç
    ...
    // --- Ã¶¾ÙËùÓÐÏÔÊ¾Æ÷²¢ÕÒµ½Ä¿±êÏÔÊ¾Æ÷ ---
    g_monitorCount = 0; // ÖØÖÃ¼ÆÊýÆ÷
    ...
    ReleaseDC(NULL, hdcScreen); // ÊÍ·ÅÆÁÄ»DC
    ...
    switch(msg) {
        case WM_TIMER: {
            if (wParam == TIMER_ID) if(update==true||got_para==false){
                // ÐÞ¸ÄÎ»Í¼Êý¾Ý
                UpdateBitmapPixels();
                // ´¥·¢´°¿ÚÖØ»æ£¬InvalidateRect»áÊ¹Õû¸ö¿Í»§ÇøÎÞÐ§
                InvalidateRect(hwnd, NULL, FALSE); // FALSE±íÊ¾±³¾°²»²Á³ý£¬¼õÉÙÉÁË¸
                //update=false;
            }
            break;
        }
        ...
        case WM_DESTROY: {
            KillTimer(hwnd, TIMER_ID); // Ïú»Ù¼ÆÊ±Æ÷
            PostQuitMessage(0);
            break;
        }
